...
def make_dataset(dataset_dir, image_size):
    ...
    counts = raw_class_counts(dataset_dir)
    missing = [name for name, count in counts.items() if count <= 0]
    if missing:
        raise RuntimeError(f"Need at least one image for every class. Missing={missing}, counts={counts}")
...
def evaluate(model, loader, device, loss_fn):
    ...
    return loss_sum / max(count, 1), 100.0 * correct / max(count, 1), confusion


def parse_args():
    parser = argparse.ArgumentParser(description="Train ResNet18 overtake front-camera classifier")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_detect_cam/dataset")
    parser.add_argument("--best-model-path", default="Overtake/overtake_detect_cam/best_overtake_detect_cam_resnet18.pth")
    parser.add_argument("--last-model-path", default="Overtake/overtake_detect_cam/last_overtake_detect_cam_resnet18.pth")
...
def main():
    ...
    if dataset.class_to_idx != EXPECTED_CLASS_TO_IDX:
        raise RuntimeError(f"Expected class mapping {EXPECTED_CLASS_TO_IDX}, got {dataset.class_to_idx}")
    ...
    print("labels: 0=none, 1=overtake")
