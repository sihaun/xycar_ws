...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ResNet18 overtake LiDAR BEV classifier")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_detect/dataset")
    parser.add_argument("--best-model-path", default="Overtake/overtake_detect/best_overtake_detect_resnet18.pth")
    parser.add_argument("--last-model-path", default="Overtake/overtake_detect/last_overtake_detect_resnet18.pth")
...
def main():
    ...
    expected = {"0_none": 0, "1_overtake": 1}
    if dataset.class_to_idx != expected:
        raise RuntimeError(f"Expected class mapping {expected}, got {dataset.class_to_idx}")
