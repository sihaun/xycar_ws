...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        any((dataset_dir / class_name).exists() and any((dataset_dir / class_name).glob("*")) for class_name in CLASS_NAMES)
        or (dataset_dir / "shortcut_cam_labels.csv").exists()
    )
    if not has_old_data:
        for class_name in CLASS_NAMES:
            (dataset_dir / class_name).mkdir(parents=True, exist_ok=True)
...
def next_labeled_sequence(dataset_dir):
    max_seq = -1
    count = 0
    pattern = re.compile(r"^shortcut_cam_(\d{6})_(blocked|none|open)\.jpg$")
    for class_name in CLASS_NAMES:
...
def make_labeled_filename(label_name, sequence):
    return f"shortcut_cam_{sequence:06d}_{label_name}.jpg"


def draw_overlay(image, status):
    preview = image.copy()
    h, w = preview.shape[:2]
    cv2.rectangle(preview, (0, 0), (w, 88), (0, 0, 0), -1)
    ...
    cv2.putText(
        preview,
        "append labels only new raw images; relabel rebuilds labels from zero.",
        (8, 80),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 220, 255),
...
def label_images(args):
    ...
    label_csv = dataset_dir / "shortcut_cam_labels.csv"
    raw_images = find_raw_images(raw_dir)
    if not raw_images:
        raise RuntimeError(f"No raw shortcut camera images found under {raw_dir}")
    ...
    cv2.namedWindow("Shortcut Cam Label", cv2.WINDOW_NORMAL)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                ...
                cv2.imshow("Shortcut Cam Label", preview)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label shortcut camera images with y=open/go, n=blocked, x=none")
    parser.add_argument("--raw-dir", default="Shortcut/shortcut_detect_cam/raw_front_cam_dataset")
    parser.add_argument("--dataset-dir", default="Shortcut/shortcut_detect_cam/dataset_front_cam")
