...
class ShortcutCamCapture(Node):
    def __init__(self, args):
        super().__init__("shortcut_cam_dataset_capture")
        self.args = args
        ...
        if args.show:
            cv2.namedWindow("Shortcut Cam Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(Image, args.topic, self.image_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"shortcut camera capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def image_callback(self, msg):
        ...
        filename = f"shortcut_cam_{ts_name}_{self.capture_id:06d}.jpg"
        ...
        if self.args.show and now - self.last_preview_time >= self.args.preview_interval:
            ...
            cv2.imshow("Shortcut Cam Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture front camera images for shortcut blocked/open detection")
    parser.add_argument("--dataset-dir", default="Shortcut/shortcut_detect_cam/raw_front_cam_dataset")
    ...
    parser.add_argument(
        ...
        default=(0, 0, 0, 0),
        metavar=("X", "Y", "W", "H"),
        help="camera crop. Use 0 0 0 0 for full frame.",
    )
