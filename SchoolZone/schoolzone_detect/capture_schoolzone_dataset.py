...
class SchoolZoneCapture(Node):
    def __init__(self, args):
        super().__init__("schoolzone_capture")
        self.args = args
        ...
        if args.show:
            cv2.namedWindow("SchoolZone Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(Image, args.topic, self.image_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"schoolzone capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def image_callback(self, msg):
        ...
        filename = f"schoolzone_{ts_name}_{self.capture_id:06d}.jpg"
        ...
        if self.args.show and now - self.last_preview_time >= self.args.preview_interval:
            ...
            cv2.imshow("SchoolZone Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture front camera images for school zone detection")
    parser.add_argument("--dataset-dir", default="SchoolZone/schoolzone_detect/raw_front_cam_dataset")
