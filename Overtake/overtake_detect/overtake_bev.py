...
def scan_to_xy_ros(ranges, angle_min, angle_increment, min_range=0.05, max_range=20.0):
    values, mask = finite_ranges(ranges, min_range=min_range, max_range=max_range)
    if len(values) == 0:
        return np.empty(0, dtype=np.float32), np.empty(0, dtype=np.float32)
...
def scan_to_xy(ranges, angle_min=None, angle_increment=None, mode="viewer", min_range=0.05, max_range=20.0):
    if mode == "ros":
        if angle_min is None or angle_increment is None:
            raise ValueError("angle_min and angle_increment are required for mode='ros'")
        return scan_to_xy_ros(ranges, angle_min, angle_increment, min_range=min_range, max_range=max_range)
    if mode == "viewer":
...
def draw_label_overlay(image, status_text=""):
    ...
    cv2.putText(view, "front", (center[0] + 5, max(center[1] - h // 8, 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (0, 0, 255), 1, cv2.LINE_AA)
    if status_text:
        cv2.putText(view, status_text, (5, 17), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1, cv2.LINE_AA)
    return view
