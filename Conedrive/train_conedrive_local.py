...
# Local conedrive direction-vector training.
# LaneFollowing/Shortcut driving과 같은 방식으로, 이미지 하단 중앙에서 라벨 점으로 향하는
# 방향 벡터(vx, vy)를 ResNet18이 회귀하도록 학습한다.
...
def build_parser():
    parser = argparse.ArgumentParser(description="Conedrive direction-vector training with ResNet18")
    parser.add_argument("--dataset-dir", default="Conedrive/dataset_xy")
    parser.add_argument("--best-model-path", default="Conedrive/best_conedrive_direction.pth")
    parser.add_argument("--last-model-path", default="Conedrive/last_conedrive_direction.pth")
