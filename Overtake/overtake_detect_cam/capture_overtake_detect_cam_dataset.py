...
class OvertakeDetectCamCapture(Node):
    def __init__(self, args):
        super().__init__("overtake_detect_cam_capture")
        self.args = args
        ...
        if args.show:
            cv2.namedWindow("Overtake Detect Cam Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(Image, args.topic, self.image_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"overtake detect cam capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def image_callback(self, msg):
        ...
        filename = f"overtake_detect_cam_{ts_name}_{self.capture_id:06d}.jpg"
        ...
        if self.args.show and now - self.last_preview_time >= self.args.preview_interval:
            ...
            cv2.imshow("Overtake Detect Cam Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture front camera images for overtake situation detection")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_detect_cam/raw_front_cam_dataset")
...
def main():
    args = parse_args()
    rclpy.init()
    node = OvertakeDetectCamCapture(args)
    try:
        rclpy.spin(node)
