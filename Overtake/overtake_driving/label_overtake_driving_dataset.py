...
LABEL_FIELDS = ["raw_image", "labeled_image", "x", "y", "angle_deg", "labeled_at"]
...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        any(dataset_dir.glob("xy_*.jpg")) or (dataset_dir / "overtake_driving_labels.csv").exists()
    )
    if not has_old_data:
        dataset_dir.mkdir(parents=True, exist_ok=True)
        return None
...
def target_angle_deg(w, h, x, y):
    dx = float(x) - (float(w) / 2.0)
    dy = float(h) - float(y)
...
def direction_line_points(w, h, x, y):
    ...
    return (start_x, start_y), (max(0, min(w - 1, end_x)), max(0, min(h - 1, end_y)))
...
def draw_overlay(image, state, index, total, raw_path):
    ...
    for label, ratio, color in GUIDE_LINES_FROM_BOTTOM:
        ...
        cv2.putText(view, label, (w - 32, max(y - 4, 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.35, color, 1, cv2.LINE_AA)
    ...
    if state.x is not None and state.y is not None:
        ...
        angle = target_angle_deg(w, h, state.x, state.y)

    status = f"{index + 1}/{total} s:save n:skip e:delete q:quit"
    if angle is not None:
        status = f"{status} angle={angle:+.1f}"
    cv2.putText(view, status, (5, 17), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.putText(view, raw_path.name[-28:], (5, h - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (255, 255, 255), 1, cv2.LINE_AA)
    return view
...
def label_images(args):
    ...
    label_csv = dataset_dir / "overtake_driving_labels.csv"
    ...
    cv2.namedWindow("Overtake Driving Label", cv2.WINDOW_GUI_EXPANDED)
    cv2.setMouseCallback("Overtake Driving Label", mouse_callback, state)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                cv2.imshow("Overtake Driving Label", draw_overlay(image, state, index, len(raw_images), raw_path))
                ...
                if key == ord("s"):
                    ...
                    writer.writerow({
                        ...
                        "x": state.x,
                        "y": state.y,
                        "angle_deg": f"{target_angle_deg(args.image_size, args.image_size, state.x, state.y):.6f}",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label overtake front camera images for direction driving")
    parser.add_argument("--raw-dir", default="Overtake/overtake_driving/raw_front_cam_dataset")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_driving/dataset_xy")
