...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        ...
        or (dataset_dir / "traffic_light_labels.csv").exists()
    )
    if not has_old_data:
        for path in class_dirs(dataset_dir):
            path.mkdir(parents=True, exist_ok=True)
...
def next_labeled_sequence(dataset_dir):
    max_seq = -1
    count = 0
    pattern = re.compile(r"^traffic_light_(\d{6})_(0_none|1_red|2_yellow|3_green|4_red_left)\.jpg$")
...
def make_labeled_filename(label_name, sequence):
    return f"traffic_light_{sequence:06d}_{label_name}.jpg"
...
def draw_overlay(image, status):
    ...
    cv2.putText(
        preview,
        "0:none  1:red  2:yellow  3:green  4:red+left  s:skip  e:delete  q:quit",
        (8, 56),
    ...
    cv2.putText(
        preview,
        "Use relabel to rebuild labels from the beginning, append for new images only.",
        (8, 80),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.45,
        (180, 220, 255),
...
def resize_for_display(image, max_width=900, max_height=520):
    h, w = image.shape[:2]
...
def label_images(args):
    ...
    label_csv = dataset_dir / "traffic_light_labels.csv"
    raw_images = find_raw_images(raw_dir)
    if not raw_images:
        raise RuntimeError(f"No raw traffic light images found under {raw_dir}")
    ...
    cv2.namedWindow("TrafficLight Label", cv2.WINDOW_NORMAL)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                ...
                cv2.imshow("TrafficLight Label", preview)
                ...
                if key in LABELS:
                    ...
                    csv_file.flush()
                    label_sequence += 1
                    print(f"{label_id}:{label_name} saved: {output_path}")
                    break
    finally:
        csv_file.close()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label traffic light ROI images with 0..4 classes")
    parser.add_argument("--raw-dir", default="TrafficLight/raw_front_cam_dataset")
