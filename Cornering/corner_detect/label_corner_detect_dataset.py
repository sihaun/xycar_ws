...
CLASS_NAMES = ["0_none", "1_cornering"]
...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        ...
        or (dataset_dir / "corner_detect_labels.csv").exists()
    )
    if not has_old_data:
        for path in class_dirs(dataset_dir):
            path.mkdir(parents=True, exist_ok=True)
...
def next_labeled_sequence(dataset_dir):
    max_seq = -1
    count = 0
    pattern = re.compile(r"^corner_detect_(\d{6})_(0_none|1_cornering)\.jpg$")
...
def make_labeled_filename(label_name, sequence):
    return f"corner_detect_{sequence:06d}_{label_name}.jpg"
...
def draw_overlay(image, status):
    ...
    cv2.putText(
        preview,
        "0:none  1:cornering  s:skip  e:delete  q:quit",
        (8, 54),
...
def label_images(args):
    ...
    label_csv = dataset_dir / "corner_detect_labels.csv"
    raw_images = find_raw_images(raw_dir)
    if not raw_images:
        raise RuntimeError(f"No raw cornering camera detection images found under {raw_dir}")
    ...
    cv2.namedWindow("Cornering Detect Cam Label", cv2.WINDOW_NORMAL)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                ...
                cv2.imshow("Cornering Detect Cam Label", preview)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label front camera images for cornering situation detection")
    parser.add_argument("--raw-dir", default="Cornering/corner_detect/raw_front_cam_dataset")
    parser.add_argument("--dataset-dir", default="Cornering/corner_detect/dataset")
