...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        any(dataset_dir.glob("xy_*.jpg")) or (dataset_dir / "conedrive_labels.csv").exists()
    )
    if not has_old_data:
        dataset_dir.mkdir(parents=True, exist_ok=True)
        return None
...
def label_images(args):
    ...
    label_csv = dataset_dir / "conedrive_labels.csv"
    ...
    cv2.namedWindow("Conedrive Label", cv2.WINDOW_GUI_EXPANDED)
    cv2.setMouseCallback("Conedrive Label", mouse_callback, state)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                cv2.imshow("Conedrive Label", draw_overlay(image, state, index, len(raw_images), raw_path))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label ROS front camera images for conedrive")
    parser.add_argument("--raw-dir", default="Conedrive/raw_front_cam_dataset")
    parser.add_argument("--dataset-dir", default="Conedrive/dataset_xy")
