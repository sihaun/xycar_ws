...
class OvertakeDrivingFrontCamCapture(Node):
    def __init__(self, args):
        super().__init__("overtake_driving_front_cam_capture")
        self.args = args
        ...
        if args.show:
            cv2.namedWindow("Overtake Driving Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(Image, args.topic, self.image_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"overtake driving capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def image_callback(self, msg):
        ...
        filename = f"overtake_front_{ts_name}_{self.capture_id:06d}.jpg"
        ...
        if self.args.show:
            ...
            cv2.imshow("Overtake Driving Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture front camera images for overtake driving direction labels")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_driving/raw_front_cam_dataset")
...
def main():
    rclpy.init()
    node = OvertakeDrivingFrontCamCapture(parse_args())
    try:
        rclpy.spin(node)
