...
from overtake_bev import draw_label_overlay, relative_to_cwd
...
def prepare_output_dir(dataset_dir, mode):
    ...
    has_old_data = dataset_dir.exists() and (
        ...
        or (dataset_dir / "overtake_detect_labels.csv").exists()
    )
    if not has_old_data:
        for label_name in LABEL_NAMES:
            (dataset_dir / label_name).mkdir(parents=True, exist_ok=True)
...
def next_labeled_sequence(dataset_dir):
    max_seq = -1
    count = 0
    pattern = re.compile(r"^overtake_detect_(\d{6})_(0_none|1_overtake)\.png$")
    for label_name in LABEL_NAMES:
...
def make_labeled_filename(label_name, sequence):
    return f"overtake_detect_{sequence:06d}_{label_name}.png"
...
def label_images(args):
    ...
    label_csv = dataset_dir / "overtake_detect_labels.csv"
    raw_images = find_raw_images(raw_dir)
    if not raw_images:
        raise RuntimeError(f"No raw overtake BEV images found under {raw_dir}")
    ...
    cv2.namedWindow("Overtake Detect BEV Label", cv2.WINDOW_NORMAL)
    ...
    try:
        for index, raw_path in enumerate(raw_images):
            ...
            while True:
                status = f"{index + 1}/{len(raw_images)} 0:none 1:overtake s:skip e:delete q:quit"
                overlay = draw_label_overlay(image, status)
                overlay = cv2.resize(overlay, (512, 512), interpolation=cv2.INTER_NEAREST)
                cv2.imshow("Overtake Detect BEV Label", overlay)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Label overtake BEV detection images")
    parser.add_argument("--raw-dir", default="Overtake/overtake_detect/raw_bev_dataset")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_detect/dataset")
