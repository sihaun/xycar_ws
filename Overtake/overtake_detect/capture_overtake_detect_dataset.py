...
from overtake_bev import draw_label_overlay, make_occupancy_image, relative_to_cwd
...
def next_capture_id(dataset_dir):
    ...
    for path in (dataset_dir / "images").glob("*.png"):
...
class OvertakeDetectBevCapture(Node):
    def __init__(self, args):
        super().__init__("overtake_detect_bev_capture")
        self.args = args
        ...
        if args.show:
            cv2.namedWindow("Overtake Detect BEV Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(LaserScan, args.topic, self.scan_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"overtake detect BEV capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def scan_callback(self, msg):
        ...
        filename = f"overtake_detect_bev_{ts_name}_{self.capture_id:06d}.png"
        ...
        if self.args.show:
            preview = cv2.resize(draw_label_overlay(image), (512, 512), interpolation=cv2.INTER_NEAREST)
            cv2.imshow("Overtake Detect BEV Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture LiDAR BEV images for overtake situation detection")
    parser.add_argument("--dataset-dir", default="Overtake/overtake_detect/raw_bev_dataset")
...
def main():
    rclpy.init()
    node = OvertakeDetectBevCapture(parse_args())
    try:
        rclpy.spin(node)
