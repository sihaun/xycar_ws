...
class TrafficLightCapture(Node):
    ...
    def __init__(self, args):
        ...
        if args.show:
            cv2.namedWindow("TrafficLight Capture", cv2.WINDOW_NORMAL)

        self.create_subscription(Image, args.topic, self.image_callback, qos_profile_sensor_data)
        self.get_logger().info(
            f"traffic light capture started: dir={self.dataset_dir} topic={args.topic} "
    ...
    def image_callback(self, msg):
        ...
        filename = f"traffic_light_{ts_name}_{self.capture_id:06d}.jpg"
        ...
        if self.args.show and now - self.last_preview_time >= self.args.preview_interval:
            ...
            cv2.imshow("TrafficLight Capture", preview)
            cv2.waitKey(1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Capture traffic light ROI images from front camera")
    parser.add_argument("--dataset-dir", default="TrafficLight/raw_front_cam_dataset")
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=4,
        default=(160, 20, 360, 170),
        metavar=("X", "Y", "W", "H"),
        help="front camera crop. Use 0 0 0 0 for full frame.",
    )
