...
def make_dataset(dataset_dir, image_size):
    ...
    if missing:
        raise RuntimeError(
            f"Need at least one image for every shortcut camera class before training. "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ResNet18 shortcut blocked/none/open camera classifier")
    parser.add_argument("--dataset-dir", default="Shortcut/shortcut_detect_cam/dataset_front_cam")
    parser.add_argument("--best-model-path", default="Shortcut/shortcut_detect_cam/best_shortcut_cam_resnet18.pth")
    parser.add_argument("--last-model-path", default="Shortcut/shortcut_detect_cam/last_shortcut_cam_resnet18.pth")
...
def main():
    ...
    if dataset.class_to_idx != EXPECTED_CLASS_TO_IDX:
        raise RuntimeError(
            ...
            "Use dataset_front_cam/blocked, dataset_front_cam/none and dataset_front_cam/open directories."
        )
    ...
    print("labels: blocked=0, none=1, open=2")
